...
fun Map<RecipeCapability<*>, List<Content>>.containsFluid(
  targetId: ResourceLocation
): Boolean {
  return this.getOrDefault(FluidRecipeCapability.CAP, emptyList())
    .any { contentWrapper ->
      ...
      return ingredient.getStacks().any { stack ->
        ...
        return@any inputFluid == targetId
      }
    }
}

fun Map<RecipeCapability<*>, List<Content>>.containsItem(
  targetId: ResourceLocation
): Boolean {
  return this.getOrDefault(ItemRecipeCapability.CAP, emptyList())
    .any { contentWrapper ->
      ...
      return ingredient.items.any { stack ->
        ...
        return@any inputItem == targetId
      }
    }
}

fun RecipeManager.getAllGTRecipes(): List<GTRecipe> {
  return GTRegistries.RECIPE_TYPES.flatMap { recipeType ->
    this.getAllRecipesFor(recipeType)
  }
}
